...
const DashboardPage: React.FC<{
      ...
      render: ({ onClose }: { onClose: () => void }) => (
        <div className={`quick-launch-context-menu-item ${isGlassTheme ? 'text-accent dark:text-white' : 'text-gray-700 dark:text-gray-200'}`}>
          ...
          <button
            ...
            className={`ml-auto inline-flex h-5 w-10 items-center rounded-full p-0.5 transition ${isGlassTheme ? 'justify-end bg-accent' : 'bg-gray-300 dark:bg-gray-600'}`}
  ...
  if (!isAdministrator) {
    return (
      <div data-no-global-context-menu="true" onContextMenu={openPageContextMenu}>
        <div className="mb-8">
          <h1>Dashboard</h1>
        </div>
