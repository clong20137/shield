...
export function downloadTrooperDailiesChartSvg(entries: TrooperDailyReportEntry[], chartType: 'pie' | 'bar', label = 'trooper-dailies') {
  const grouped = entries.reduce<Record<string, number>>((totals, entry) => {
    const key = entry.districtWorked || 'Unknown';
    totals[key] = (totals[key] || 0) + 1;
    return totals;
  }, {});
  const rows = Object.entries(grouped).sort((a, b) => b[1] - a[1]).slice(0, 12);
  const total = rows.reduce((sum, [, value]) => sum + value, 0) || 1;
  const colors = ['#9C865C', '#1F3A5F', '#2563EB', '#16A34A', '#DC2626', '#7C3AED', '#EA580C', '#0891B2', '#4B5563', '#BE123C', '#65A30D', '#9333EA'];
  const width = 980;
  const height = 620;
  const title = chartType === 'pie' ? 'Trooper Daily Reports by District - Pie Chart' : 'Trooper Daily Reports by District - Bar Chart';

  let chart = '';
  if (chartType === 'pie') {
    let cumulative = 0;
    chart = rows.map(([name, value], index) => {
      ...
      return `<path d="M ${cx} ${cy} L ${x1.toFixed(2)} ${y1.toFixed(2)} A ${radius} ${radius} 0 ${largeArc} 1 ${x2.toFixed(2)} ${y2.toFixed(2)} Z" fill="${colors[index % colors.length]}"><title>${name}: ${value}</title></path>`;
    }).join('');
  } else {
    const maxValue = Math.max(...rows.map(([, value]) => value), 1);
    chart = rows.map(([name, value], index) => {
      const barWidth = (value / maxValue) * 560;
      const y = 135 + index * 36;
      return `<text x="40" y="${y + 18}" font-size="13" font-family="Arial" fill="#1F2937">${escapeXml(name)}</text><rect x="300" y="${y}" width="${barWidth.toFixed(2)}" height="24" rx="5" fill="${colors[index % colors.length]}"/><text x="${310 + barWidth}" y="${y + 17}" font-size="13" font-family="Arial" font-weight="700" fill="#1F2937">${value}</text>`;
    }).join('');
  }

  const legend = rows.map(([name, value], index) => {
    const y = 165 + index * 28;
    return `<rect x="620" y="${y - 12}" width="14" height="14" rx="3" fill="${colors[index % colors.length]}"/><text x="642" y="${y}" font-size="13" font-family="Arial" fill="#1F2937">${escapeXml(name)} (${value})</text>`;
  }).join('');

  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
    <rect width="100%" height="100%" fill="#F8FAFC"/>
    <text x="40" y="58" font-size="26" font-family="Arial" font-weight="700" fill="#1F3A5F">${title}</text>
    <text x="40" y="86" font-size="13" font-family="Arial" fill="#64748B">Generated by SHIELD - ${new Date().toLocaleString()} - ${entries.length} reports</text>
    ${chart}
  ...
  link.download = `${label}-${chartType}-chart.svg`;
  link.click();
